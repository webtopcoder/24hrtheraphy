...
class MessageList extends PureComponent<IProps> {
  ...
  async handleScroll(conversation, event) {
    const { message, loadMoreStreamMessages: dispatchLoadMoreStreamMessages } =
      this.props;
  ...
  onbeforeunload = () => {
    const { conversation, resetStreamMessage: dispatchResetStreamMessage } =
      this.props;
    if (conversation && conversation._id) {
  ...
  renderMessages = () => {
    const { message, currentUser, currentPerformer, conversation, loggedIn } =
      this.props;
    const messages = message.items;
