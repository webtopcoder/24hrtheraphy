...
class UserTokensPage extends PureComponent<IProps, IStates> {
  ...
  componentDidMount() { this.getTokens(); }
  ...
  render() {
    ...
    return (
        <><Head><title> Funds - Tokens</ title></ Head>
        <div className = "funds-tokens-box">
        <PageHeader title = "Buy More Tokens" />
        <div className = "tokens-section"><div className = "tokens-card">
        <Row>{fetching
                  ? (<Loader />)
                  : tokens && tokens.length
                        ? (tokens.map(
                              (item) =
                                  > (<Col xs = {12} md = {8} xl = {6} xxl =
                                          {4} key = {item._id}>
                                     <TokenCard name = {item.name} token =
                                          {item.tokens} price =
                                              {item.price} description =
                                                  {item.description} onBuyToken =
                                      {
                                        () = > {
        ...
        <Modal visible = {open} title = "Buy Now" onCancel =
         {
           () = > { this.setState({open : false}); }
         } footer = {[]}>{
            selectedToken
                ? (<>
                   <PaypalPayment onApprove =
                    {
                      async(tokenizedData) = > {
                    ...
                    } amount = {selectedToken.price} forceReRender =
                        {[selectedToken]} />
                   <CoinbasePayment tokenPackage = {selectedToken._id} /></>)
                : ("No selected token")}</ Modal></ div></ div></ div></>);
  }
